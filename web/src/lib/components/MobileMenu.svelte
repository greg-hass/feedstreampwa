...
{#if $isMobileMenuOpen}
  ...
  <!-- Slide-in Menu -->
  <aside
    class="fixed left-0 top-0 h-full w-[280px] bg-background border-r border-white/10 z-[70] md:hidden overflow-y-auto shadow-2xl"
  ...
  >
    ...
    <div class="px-4 py-2 space-y-1">
      ...
      <button
        class="w-full flex items-center justify-between gap-3 px-3 py-2.5 text-sm font-medium rounded-xl transition-all duration-200 group relative overflow-hidden
          {$viewMode === 'smart' && $activeSmartFolder === 'rss'
            ? 'bg-white/10 text-white shadow-inner border border-white/5'
            : 'text-white/60 hover:text-white hover:bg-white/5'}"
        on:click={() => { setViewSmartFolder('rss'); closeMenu(); }}
      >
        <div class="flex items-center gap-3">
          <Rss size={24} class={$viewMode === 'smart' && $activeSmartFolder === 'rss' ? 'text-accent' : 'text-current group-hover:text-white'} />
          RSS
        </div>
        {#if $rssCount.total > 0}
      ...
      <button
        class="w-full flex items-center justify-between gap-3 px-3 py-2.5 text-sm font-medium rounded-xl transition-all duration-200 group relative overflow-hidden
          {$viewMode === 'smart' && $activeSmartFolder === 'youtube'
            ? 'bg-white/10 text-white shadow-inner border border-white/5'
            : 'text-white/60 hover:text-white hover:bg-white/5'}"
        on:click={() => { setViewSmartFolder('youtube'); closeMenu(); }}
      >
        <div class="flex items-center gap-3">
          <Youtube size={24} class={$viewMode === 'smart' && $activeSmartFolder === 'youtube' ? 'text-accent' : 'text-current group-hover:text-white'} />
          YouTube
        </div>
      ...
      <button
        class="w-full flex items-center justify-between gap-3 px-3 py-2.5 text-sm font-medium rounded-xl transition-all duration-200 group relative overflow-hidden
          {$viewMode === 'smart' && $activeSmartFolder === 'reddit'
            ? 'bg-white/10 text-white shadow-inner border border-white/5'
            : 'text-white/60 hover:text-white hover:bg-white/5'}"
        on:click={() => { setViewSmartFolder('reddit'); closeMenu(); }}
      >
        <div class="flex items-center gap-3">
          <MessageCircle size={24} class={$viewMode === 'smart' && $activeSmartFolder === 'reddit' ? 'text-accent' : 'text-current group-hover:text-white'} />
          Reddit
        </div>
      ...
      <button
        class="w-full flex items-center justify-between gap-3 px-3 py-2.5 text-sm font-medium rounded-xl transition-all duration-200 group relative overflow-hidden
          {$viewMode === 'smart' && $activeSmartFolder === 'podcast'
            ? 'bg-white/10 text-white shadow-inner border border-white/5'
            : 'text-white/60 hover:text-white hover:bg-white/5'}"
        on:click={() => { setViewSmartFolder('podcast'); closeMenu(); }}
      >
        <div class="flex items-center gap-3">
          <Mic size={24} class={$viewMode === 'smart' && $activeSmartFolder === 'podcast' ? 'text-accent' : 'text-current group-hover:text-white'} />
          Podcasts
        </div>
      ...
      <!-- Feeds & Folders Tree -->
      <div class="text-xs font-semibold text-white/40 uppercase tracking-wider px-3 mb-2 mt-8">
        Feeds & Folders
      </div>
            ...
            <button
              class="w-full flex items-center justify-between gap-2 px-3 py-2.5 text-sm font-medium rounded-xl transition-all duration-200 group relative overflow-hidden
                {$viewMode === 'folder' && $activeFolderId === folder.id
                  ? 'bg-white/10 text-white shadow-inner border border-white/5'
                  : 'text-white/60 hover:text-white hover:bg-white/5'}"
              on:click={() => { setViewFolder(folder.id); closeMenu(); }}
            >
              ...
              <div class="flex flex-col ml-3 pl-3 border-l border-white/5 mt-1 mb-1 gap-1">
                {#each $feedsTree.byFolder[folder.id] || [] as feed}
                  <button
                    ...
                    on:click={() => { setViewFeed(feed.url); closeMenu(); }}
                  >
