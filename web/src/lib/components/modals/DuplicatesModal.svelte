<script lang="ts">
  import { X, Copy, Trash2, CheckCircle2, AlertCircle, Eye, EyeOff, Settings2, RefreshCw } from "lucide-svelte";
  ...
  async function removeDuplicates(group: any) {
    ...
    toast.show(`Removed ${removed} duplicate${removed !== 1 ? 's' : ''}`, "success");
...
<div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
  <div class="w-full max-w-4xl bg-[#18181b] rounded-2xl shadow-2xl border border-white/10 flex flex-col max-h-[90vh]">
    <!-- Header -->
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-sm text-white/50">
            {$duplicateGroupCount} group{$duplicateGroupCount !== 1 ? 's' : ''} ·
      ...
      {#if showSettings}
        ...
        <div class="p-6 space-y-6">
          <div class="flex items-center justify-between">
            <h3 class="text-lg font-semibold text-white">Duplicate Detection Settings</h3>
            <button
          ...
          <div class="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/5">
            <div>
              <div class="font-medium text-white">Enable Duplicate Detection</div>
              <div class="text-sm text-white/50">Automatically find duplicate articles</div>
            </div>
            <button
          ...
          <!-- Detection Method -->
          <div class="space-y-3">
            <label class="text-sm font-semibold text-white">Detection Method</label>
            <div class="grid grid-cols-3 gap-3">
              {#each ['url', 'title', 'fuzzy'] as method}
                <button
                  class="p-4 rounded-xl border-2 transition-all {$duplicateSettings.method === method
                    ? 'border-accent bg-accent/10'
                    : 'border-white/10 bg-white/5 hover:bg-white/10'}"
                  on:click={() => duplicateSettings.setMethod(method)}
                >
                  <div class="text-sm font-medium text-white capitalize">{method}</div>
                  <div class="text-xs text-white/50 mt-1">
                    {method === 'url' ? 'Exact URL match' : method === 'title' ? 'Exact title match' : 'Similar titles'}
                  </div>
                </button>
              {/each}
            </div>
          ...
          {#if $duplicateSettings.method === 'fuzzy'}
            <div class="space-y-3">
              <div class="flex items-center justify-between">
                <label class="text-sm font-semibold text-white">Sensitivity</label>
                <span class="text-sm text-accent">{$duplicateSettings.sensitivity}%</span>
              </div>
              <input
                type="range"
                min="50"
                ...
                on:input={(e) => duplicateSettings.setSensitivity(parseInt(e.target.value))}
          ...
          <!-- Keep Preference -->
          <div class="space-y-3">
            <label class="text-sm font-semibold text-white">Which duplicate to keep?</label>
            <select
              value={$duplicateSettings.keepInFeed}
              on:change={(e) => duplicateSettings.setKeepInFeed(e.target.value)}
              class="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white outline-none focus:border-accent/50"
            >
              <option value="first_subscribed">From first subscribed feed</option>
          ...
          <!-- Auto Action -->
          <div class="space-y-3">
            <label class="text-sm font-semibold text-white">Auto-action for duplicates</label>
            <div class="grid grid-cols-3 gap-3">
              {#each ['none', 'mark_read', 'hide'] as action}
                <button
                  class="p-4 rounded-xl border-2 transition-all {$duplicateSettings.autoAction === action
                    ? 'border-accent bg-accent/10'
                    : 'border-white/10 bg-white/5 hover:bg-white/10'}"
                ...
                >
                  ...
                  <div class="text-xs text-white/50 mt-1">
                    {action === 'none' ? 'Show all' : action === 'mark_read' ? 'Mark as read' : 'Hide from view'}
                  </div>
                </button>
              {/each}
            </div>
          ...
          {#if $duplicates.length === 0}
            ...
            <div class="text-center py-12">
              ...
              <p class="text-white/50">
                Click "Scan" to check for duplicate articles based on your settings.
              </p>
            </div>
          {:else}
                ...
                <div class="bg-white/5 rounded-xl border border-white/10 overflow-hidden">
                  ...
                  <div class="px-4 py-3 bg-white/5 border-b border-white/10 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                      ...
                      <span class="font-medium text-white">{group.representativeTitle}</span>
                      <span class="px-2 py-0.5 rounded-full bg-orange-500/20 border border-orange-500/30 text-xs font-medium text-orange-400">
                  ...
                  <div class="divide-y divide-white/5">
                    {#each group.items as item}
                      <div class="px-4 py-3 flex items-start gap-3 hover:bg-white/5 transition-colors group/item">
                        <div class="flex-1 min-w-0">
                          ...
                          <p class="text-sm text-white/80 line-clamp-2">{item.title}</p>
                          <p class="text-xs text-white/40 mt-1 truncate">{item.url}</p>
                        </div>
                        <button
