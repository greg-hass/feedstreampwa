...
export default async function refreshRoutes(fastify: any, options: any) {
    // Start refresh job
    fastify.post('/refresh/start', async (request: any, reply: any) => {
        // limit concurrent jobs
        if (refreshJobs.size >= MAX_JOBS) {
            // cleanup old jobs
            const now = Date.now();
            for (const [id, job] of refreshJobs.entries()) {
                if (now - job.startedAt > 5 * 60 * 1000) { // 5 mins TTL
        ...
        const specificUrls = body?.urls; // Optional array of URLs to refresh
        ...
        try {
            ...
            if (urlsToRefresh.length === 0) {
                return { ok: true, jobId: null, message: 'No feeds to refresh' };
            }
    ...
    fastify.get('/refresh/status', async (request: any, reply: any) => {
        ...
        if (!jobId || !refreshJobs.has(jobId)) {
            // If job not found, assume done if it's an old ID, or invalid
